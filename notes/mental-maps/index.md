...
Anyway, I'm still kind of stabbing around in the dark with this stuff, and I've got way more questions than answers. Beyond the question of how to pick words, a couple of things that I'm curious about:

1. The layouts are very sensitive to the "skim depth" - the number of connections that get pulled off the top of the distribution topics and added to the graph as edges. Too few, and the network is fragmented and disconnected. Too many, and the layout is a big scrambled ball of words. Again and again, 10 seems to be the magic number, the goldilocks zone where the words bind together into the most interpretable clumps. Why is this? What's actually being measured, here? Is it universally true that words tend to hang together with about 10 other words in a long document, but no more and no less?

1. Is there a better way to compute the similarity between the probability distribution functions? Right now, I'm using the [Bray-Curtis dissimilarity](http://en.wikipedia.org/wiki/Bray%E2%80%93Curtis_dissimilarity) test in [scipy](http://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.spatial.distance.braycurtis.html), which is fast an easy to reason about. But, as I mentioned before, there are dozens of different ways to do this, and I don't really understand how they differ or what's at stake. Is there a different way of doing this that might produce significantly better results?

1. The visualizations are fun, but, in the long term, I'm actually more curious about whether this kinds of networks might offer up new ways to _classify_ texts. Just eyeballing the layouts, two things come to mind. First, some notion of "modularity" of clumpiness - the extent to which the text partitions fof into discrete, self-contained regions of meaning (_War and Peace_). And, related but somewhat different, the "width" or "breadth" of the text, the degree to which it semantically drifts or ranges - the maximum distance between any two nodes in the network. For example, the _Divine Comedy_, which is very "wide" - it's a long trip from "torment" at the bottom to "Christ" at the top - but not especially "clumpy" relative to something like the _Odyssey_ or _Notes from Underground_. How would these scores vary from text to text, author to author, genre to genre, period to period? It seems like an interesting way to quantify the intuition that the novel, for instance, is defined by a kind of internal variety - a plurality of plot lines, characters, etc. Which novels, novelists, periods, are the clumpiest? How do the shapes of texts change over time?
