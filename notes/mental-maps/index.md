...
1. The layouts are extremely sensitive to the "skim depth" - the number of connections that get pulled off the top of the distribution topics and added to the graph as edges. Too few, and the network is fragmented and disconnected. Too many, and the layout is scrambled, a big ball of words that doesn't really surface anything interesting about the text. So far, 10 seems seems to be the magic number, the sweet spot where the words bind together into the most interpretable clumps. Why is this? What's actually being measured, here? Is it just universally true that words tend to hang together in groups of 10, but no more and no less? Or does this vary - perhaps in interesting ways - from text to text, author to author, etc.?
