...
Anyway, I'm still kind of stabbing around in the dark with stuff. The visualizations are fun as "deformations" of the texts, to use Steve Ramsay's word - derivative texts, the products of an alrogithmic sieve that can confirm or challenge intuitions about the originals. But, in the long run, I'm really more interested in the question of whether this kind of network informaion could be tapped to get a more general-purpose insight about the "shapes" of texts in the aggregate. The question becomes - how to move beyond the visual close reading (fun and even productive as it might be) and find a way to classify and compare the networks, in the same way that something like the Bray-Curtis dissimilarity makes it possible to operationalize the probability density functions of the individual words? Are there as many shapes as there are texts, or can the networks be computationally slice and diced into a smaller set of archetypes? For example, the _Divine Comedy_ and _Walden_ just _look_ almost identical to the eye - but how to capture that quantitatively? What exactly is the underlying similarity? Is it something real, or is it just a coincidence? Could is be boiled out as some kind of network-theoretical measurement? Maybe some notion of the "width" or "breadth" of the text, the maximum distance between nodes, the extent to which the text traverses across a semantic space without looping back on itself? If this is computable - what other texts are "wide" or "long" in this way? Do they cohere at a literary level? Do they all tend to be "oppositional" or "dialectical" texts like the _Divine Comedy_ and _Walden_ - heaven/hell, nature/wilderness, land/water? Are they all "travelogues," in a conceptual sense - narratives about a continuous, gradual movement from one pole to the other, which prevents the network from folding back on itself and wiring up a shorter distance between the most removed terms? Or what about some measure of "modularity" or "clumpiness," the extent to which a text separates out into discrete, self-contained little circuits of meaning? If patterns do exist - (how) do they correlate with individual authors, periods, regions, genres, gender, nationality, identity? How do they change over time?
